...
router.get('/', function(req, res, next) {
  ...
  user.search3(keyword,invitedmemno,invitedmemno2,postmemno,postmemno2,memno10,memno11).then(data => {
    console.log(data)
    if(data==null){
        res.render('error');  //導向錯誤頁面
    }else if(data.friend.length > 0){
      res.render('searchList', {results:data});  //將資料傳給顯示頁面
    }else{
